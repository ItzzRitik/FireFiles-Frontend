...
const LoginModal = (props) => {
	const [showSignUp, setShowSignUp] = React.useState(false),
		...
		onOverlayClick = () => {
			...
			if (props.action === 'login') return history.push('/signup');
	...
	React.useEffect(() => {
		if (props.action === 'login') setShowSignUp(false);
		else if (props.action === 'signup') setShowSignUp(true);
	}, [props.action]);

	return (
		!props.action ?
			<div /> :
			<div className={'loginPage ' + (!animateSlide ? 'pauseTransition' : '')}>
